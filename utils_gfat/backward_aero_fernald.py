# -*- coding: utf-8 -*-
"""
Created on Tue Jan  5 17:06:54 2021

@author: Hengheng Zhang

E-Mail: hengheng.zhang@kit.edu

Function： BackwardAeroFernald 
...
# from Src.backward_integration_backscatter import backward_integration_backscatter
...
class backward_aero_fernald:
    ...
    def __init__(self,DataInformation,StationGeoInfo): 
        ...
        if 'lamda' in DataInformation.keys():
            self.lamda = DataInformation['lamda']
        else:
            self.lamda = 355
    ...
    def __call__(self,Use_Radio_Sonde = False):
        ...
        if Use_Radio_Sonde:
            self.BetaMol = molecule_scatter(self.baseLatitude,self.baseLongitude,self.Attitude+self.baseAlt,self.Date,
                                          ...
                                          InputSelection='radio_sonde',lamda = self.lamda)
        else:
            self.BetaMol = molecule_scatter(self.baseLatitude,self.baseLongitude,self.Attitude+self.baseAlt,self.Date,lamda = self.lamda)
       
        self.AlphaMol = self.BetaMol*self.Sm
        
        ## Calculate Aerosol Backscattering 
        # self.BetaAero=backward_integration_backscatter(self.Sa,self.Sm,self.R_zc,self.RangeIndex,self.Range,self.AlphaMol,self.RCS)    
        self.BetaAero = lidar_elastic_retrieval.klett(self.RCS, self.Range, self.BetaMol, self.Sm,
                                      lr_aer = self.Sa, ymin = self.ReferenceHeight - 250, ymax = self.ReferenceHeight + 250,aerosol_backscatter_at_reference=self.R_zc)
    ...
    def getBetaRatio(self):
        """
            get Aerosol backacattering ratio
            
        """
        return (self.BetaAero/self.BetaMol[:len(self.BetaAero)])+1
