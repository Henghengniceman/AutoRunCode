# -*- coding: utf-8 -*-
"""
Created on Fri Jul 15 22:09:31 2022
...
from MultiZenithScanningMeasurement import MultiZenithScanningMeasurement
import pandas as pd
import numpy as np
import xarray as xr
...
class WorkFlow(object):
    def __init__(self,MainPath,StartDate,EndDate):
        self.ConfigInfo = {} 
    ...
    def ZenithScanning(self):
        ...
        # ZenithDataPaths = ZenithDataPaths[0:1]
        # pdb.set_trace()
    ...
    def MultiZenithScanning(self):
        print('\n-----Multiple ZenithScanning start-----\n')
        MultiZenithDataPaths =[]
        for year in os.listdir(self.MainPath):
                        ...
                        if (month.startswith('0') or month.startswith('1')):
                            for day in os.listdir(self.MainPath +'/' + year + '/Scanning_Measurements/'+month):
                                if (day.startswith('0') or day.startswith('1') or day.startswith('2') or day.startswith('3')):
                                    ...
                                    if float(year+month+day) >= float(self.ConfigInfo['StartDate'].strftime('%Y%m%d'))and float(year+month+day)  <= float(self.ConfigInfo['EndDate'].strftime('%Y%m%d')):
                                        for Time in os.listdir(self.MainPath +'/' + year + '/Scanning_Measurements/'+month+'/'+day):
                                            if Time.startswith('3D'):
                                                MultiZenithDataPaths.append(self.MainPath +'/' + year + '/Scanning_Measurements/'+month+'/'+day+'/'+Time)
        # import pdb
        # pdb.set_trace()
        BetaSets = pd.DataFrame()
        DeltaSets = pd.DataFrame()
        BetaRetrieved = 0
        DepolarizationRetrieved = 0
        # MultiZenithDataPaths = MultiZenithDataPaths[0:1]
        for datepath in tqdm(MultiZenithDataPaths):
            try:
                MZSM = MultiZenithScanningMeasurement(datepath,self.ConfigInfo)
                MZSM.ReadData()
                if MZSM.BinWidthCheck and MZSM.FileNames != []:
                    MZSM.DataPreProcess()
                    if self.ConfigInfo['Overlap']:
                        MZSM.OverlapCorrection()
                    MZSM.GroupData()
                    if self.ConfigInfo['RCSProfiles']:
                        MZSM.ShowRCSProfiles()
                    if self.ConfigInfo['RCSPolar']:
                        MZSM.ShowRCSPolar()
                    if self.ConfigInfo['BetaProfiles'] or self.ConfigInfo['BetaPolar'] or self.ConfigInfo['BetaData']:
                        MZSM.AeroFernald()
                        ...
                        BetaRetrieved = 1
                    if self.ConfigInfo['BetaProfiles']:
                        MZSM.ShowBetaProfiles()
                    if self.ConfigInfo['BetaPolar']:
                        MZSM.ShowBetaPolar()
                    if self.ConfigInfo['BetaData']:
                        MZSM.SaveBeta()
                    if self.ConfigInfo['DeltaProfiles'] or self.ConfigInfo['DeltaPolar'] or self.ConfigInfo['DeltaData']:
                        MZSM.DepolarizationCal()
                        DepolarizationRetrieved = 1
                    if self.ConfigInfo['DeltaProfiles'] or self.ConfigInfo['DeltaPolar'] or self.ConfigInfo['DeltaData']:
                        MZSM.ShowDepolarizationProfiles()
                    if self.ConfigInfo['DeltaPolar']:
                        MZSM.ShowDepolarizationPolar()
                    if self.ConfigInfo['DeltaData']:
                        MZSM.SaveDepolarization()
                    #%% contour plot
                    if self.ConfigInfo['ZenithContourf'] and BetaRetrieved and DepolarizationRetrieved:
                        BetaSet,deltaSet = MZSM.GetContourfData()   
                        BetaSets=pd.concat([BetaSets,BetaSet],axis=1)
                        DeltaSets=pd.concat([DeltaSets,deltaSet],axis=1)
                    self.BetaSets = BetaSets
                    self.DeltaSets = DeltaSets
                ...
                pass
        if len(MultiZenithDataPaths)>0:
            loginfo = self.loginfo+'\n'+str(len(MultiZenithDataPaths))+' Groups Data was analysised'
            WriteLogFile(loginfo,self.Date,'ZS_')
...
if __name__ == '__main__':
    ...
    StartDate = datetime. strptime(args.date_ini_str, '%Y%m%d')
    EndData = datetime. strptime(args.date_end_str, '%Y%m%d')
    
    # StartDate = datetime. strptime('20220804', '%Y%m%d')
    # EndData = datetime. strptime('20220804', '%Y%m%d')
    dirname,filename = os.path.split(os.path.abspath(__file__))
    ConfigFilePath = os.path.join(dirname,'Config.init')
    workflowI = WorkFlow(MainPath,StartDate,EndData)
    workflowI.ReadConfigFile(ConfigFilePath)
    workflowI.ZenithScanning()
